...
export const getAdminQueue = async ({
  ...
  try {
    ...
    // Exact reason string, so "add the XHORSASIS material" is one filtered
    // pass over every garment blocked on it rather than 29 separate hunts.
    if (reason) query["ingest.review.reasons"] = reason;
    if (source) query["ingest.source"] = source;
...
/**
 * Reason counts across the whole queue, not just the current page — the
 * point is to see that 29 garments share one blocker before you start
 * clicking through them one at a time.
 */
export const getReviewReasonCounts = async ({ status = "pending" } = {}) => {
  try {
    await initMongo();
    const rows = await Garment.aggregate([
      { $match: { status, "ingest.review.reasons.0": { $exists: true } } },
      { $unwind: "$ingest.review.reasons" },
      { $group: { _id: "$ingest.review.reasons", count: { $sum: 1 } } },
      { $sort: { count: -1 } },
      { $limit: 40 },
    ]);
    return rows.map((r) => ({ reason: r._id, count: r.count }));
  } catch (error) {
    console.error("Error counting review reasons:", error);
    return [];
  }
};

/** Distinct ingest sources present in a queue, for the source filter. */
export const getQueueSources = async ({ status = "pending" } = {}) => {
  try {
    await initMongo();
    const rows = await Garment.aggregate([
      { $match: { status, "ingest.source": { $exists: true } } },
      { $group: { _id: "$ingest.source", count: { $sum: 1 } } },
      { $sort: { count: -1 } },
    ]);
    return rows.map((r) => ({ source: r._id, count: r.count }));
  } catch (error) {
    console.error("Error counting queue sources:", error);
    return [];
  }
};
