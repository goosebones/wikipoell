...
/**
 * Accepts `"url"` or `{ url, sourceUrl }` and returns image subdocuments,
 * de-duplicated on sourceUrl where present (falling back to url).
 */
function normalizeImages(images, { required }) {
  const seen = new Set();
  const list = [];
  for (const item of Array.isArray(images) ? images : []) {
    ...
    seen.add(key);
    list.push({ url, ...(sourceUrl && { sourceUrl }) });
  }
...
export async function listIngestGarments(source) {
  ...
  return rows.map((g) => ({
    ...
    // So the pipeline can skip re-downloading images it already copied.
    imageSourceUrls: (g.images ?? [])
      .map((i) => i.sourceUrl)
      .filter((u) => typeof u === "string"),
    // Images from before the pipeline have no sourceUrl. When the counts
    // differ, the client cannot tell what it already has and must not copy.
...
export async function createIngestGarment(body) {
  ...
  const images = normalizeImages(body.images, { required: true });
  const now = new Date();
...
export async function updateIngestGarment(id, body) {
  ...
  // Dedupe on sourceUrl: each R2 upload gets a fresh UUID url, so comparing
  // final urls would append the same image on every re-scrape.
  const have = new Set(
    (garment.images ?? []).flatMap((i) => [i.sourceUrl, i.url].filter(Boolean)),
  );
  // Images predating the pipeline carry no sourceUrl, so there is nothing to
  // match a source image against — and appending anyway means re-copying the
  // entire set on every run. That is exactly what happened to the 478
  // migrated garments the first time this ran. When any image is unmatchable,
  // append nothing: a garment that already has pictures loses less by missing
  // a new one than by doubling the ones it has.
  const unmatchable = (garment.images ?? []).some((i) => !i.sourceUrl);
  const newImages = unmatchable
    ? []
    : normalizeImages(body.images, { required: false }).filter(
        (img) => !have.has(img.sourceUrl ?? img.url),
      );
  ...
  return {
    ...
    ...(unmatchable && {
      imagesSkipped: "pre-pipeline images have no sourceUrl",
    }),
  };
}
